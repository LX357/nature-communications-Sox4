...
use strict;
use Bio::DB::Fasta;
if(scalar(@ARGV)!=10){die "\nInputs are <BAM_file><window_size><foldchange for pause><reference fasta file><comma separated read lengths><window coverage><upstream sequence><downstream sequence><comma separated offset_value><outfile>\n\n";}
my $bam_file=$ARGV[0];
my $window = $ARGV[1];
...
####Annotation_file
my %annotation;
open FA,"$fasta_file" or die $!;
while(<FA>){
	if(/^>/ && /CDS/){
		my $trans = $_;
		$trans =~ s/>//g;
		$trans =~ s/\s.*//g;
		my ($cds_start,$cds_end) = $_ =~ /CDS=(\d+)-(\d+)/;
		$annotation{$trans}=[$cds_start,$cds_end];
	}
}
my $outfile_codon= $outfile;
print $outfile_codon."\n";
$outfile_codon =~ s/\.xls$//;
$outfile_codon = "$outfile_codon.annot.xls";
...
foreach my $trans (sort {$a cmp $b} keys %annotation)
{
	foreach my $key (keys %uniq_zscore_values){#print "@{$uniq_zscore_values{$key}}->[0]\t$array[0]\n";
		...
		my ($start,$end);
		if($uniq_zscore_values{$key}[0] eq $trans)
		{
			$start=$annotation{$trans}[0];$end=$annotation{$trans}[1];
			...
			for(my $i=$start;$i<=$end;$i+=3)
			{
				...
				{
					my $seq = $db->seq($trans,$codonstrt, $codonend);
